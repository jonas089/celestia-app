...
if [ ! -f "$GENESIS_FILE" ]; then
    ...
    celestia-appd init zkevm-test --chain-id celestia-zkevm
    celestia-appd config set client chain-id celestia-zkevm
    ...
    # Bind the priv-validator gRPC to all interfaces so the fibre server (running
    # in a SEPARATE container) can reach it to sign payment promises. It defaults
    # to 127.0.0.1:26669, which is unreachable across the docker network.
    sed -i 's#priv_validator_grpc_laddr = "127.0.0.1:26669"#priv_validator_grpc_laddr = "0.0.0.0:26669"#' config/config.toml
